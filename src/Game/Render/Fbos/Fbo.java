...
public class Fbo {
	...
	private boolean isMultisampledAndMultiRenderTargets = false;
	...
	public Fbo(int width, int height) {
		this.width = width;
		this.height = height;
		this.isMultisampledAndMultiRenderTargets = true;
	...
	public int getColorBuffer2(){
		return colourBuffer2;
	}
	...
	public void resolveToFbo(int readBuffer,Fbo fbo){
	...
	private void initialiseFrameBuffer(int type) {
		createFrameBuffer();
		if(isMultisampledAndMultiRenderTargets){
			createMultiColorBufferAttachment();
		}else{
	...
	private void createMultiColorBufferAttachment(){
		...
		GL30.glFramebufferRenderbuffer(GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0, GL30.GL_RENDERBUFFER,
				colourBuffer);
		colourBuffer2 = GL30.glGenRenderbuffers();
		GL30.glBindRenderbuffer(GL30.GL_RENDERBUFFER, colourBuffer2);
		GL30.glRenderbufferStorage(GL30.GL_RENDERBUFFER, GL11.GL_RGBA8, width, height);
